...
DWORD WINAPI CClient::_ConnectionThread(LPVOID lpParam)
{
	...
	pClient->ShowMessage("_AcceptThread start, listening...\n");
	pClient->EstablishConnections();
	pClient->ShowMessage("_ConnectionThread is over.\n");
...
DWORD WINAPI CClient::_WorkerThread(LPVOID lpParam)
{
	...
	{
		pClient->ShowMessage(_T("Test concurrent %d threads is finished."),
			pClient->m_nThreads);
	}
...
bool CClient::EstablishConnections()
{
	...
	for (int i = 0; i < m_nThreads; i++)
	{
		...
		{
			ShowMessage("Received shutdown command from user.\n");
			return true;
		}
		// 向服务器进行连接
		...
		{
			ShowMessage(_T("Connect server failed"));
...
bool CClient::ConnectToServer(SOCKET& pSocket, CString strServer, int nPort)
{
	...
	{
		ShowMessage("Socket initialization failed, err=%d\n",
			WSAGetLastError());
		pSocket = NULL;
		return false;
	}
	...
	{
		ShowMessage("Failed to connect to server, err=%d\n",
			WSAGetLastError());
		closesocket(pSocket);
		pSocket = NULL;
		return false;
...
bool CClient::LoadSocketLib()
{
	...
	if (NO_ERROR != nResult)
	{
		ShowMessage(_T("Init WinSock 2.2 failed!\n"));
		return false; // 错误
	}
	return true;
}
