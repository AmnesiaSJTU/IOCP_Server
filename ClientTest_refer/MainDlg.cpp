...
void CMainDlg::InitGUI()
{
	...
	{
		AfxMessageBox(_T("Loading Winsock 2.2 failed，can't run server！"));
		PostQuitMessage(0);
	}
	// 设置本机IP地址
...
void CMainDlg::OnBnClickedOk()
{
	...
	if (strIP == _T("") || strMessage == _T("")
		|| nPort <= 0 || nThreads <= 0)
	{
		AfxMessageBox(_T("Please input invalid params！"));
		return;
	}
	CListCtrl* pList = (CListCtrl*)GetDlgItem(IDC_LIST_INFO);
	...
	m_Client.ShowMessage("Start the test");
...
void CMainDlg::OnBnClickedStop()
{
	...
	m_Client.ShowMessage("Stop the test");
}
